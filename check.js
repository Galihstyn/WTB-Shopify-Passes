...
export default function App() {
  ...
  return (
    <div className="relative h-screen w-full bg-[#0a0f1d] text-slate-200 overflow-hidden font-sans select-none touch-none">
      ...
      {/* Utility Panel Kiri (Undo, Redo, Zoom, Clear) */}
      <div
        className={`fixed left-4 bg-[#1e293b]/90 backdrop-blur-xl border border-slate-700/50 shadow-2xl flex flex-col items-center z-50 transition-all duration-500 h-max py-4 w-12 md:w-14 rounded-2xl gap-3
          ${(isBottomPanelOpen || selectedIds.length > 0) ? 'top-[35%] md:top-1/2 -translate-y-1/2' : 'top-1/2 -translate-y-1/2'}`}
      >
        <button onClick={undo} disabled={historyIndex === 0} title="Undo" className="w-9 h-9 flex items-center justify-center hover:bg-slate-700 rounded-lg disabled:opacity-20 text-indigo-400 transition-all"><Undo2 size={16} /></button>
        <button onClick={redo} disabled={historyIndex === history.length - 1} title="Redo" className="w-9 h-9 flex items-center justify-center hover:bg-slate-700 rounded-lg disabled:opacity-20 text-indigo-400 transition-all"><Redo2 size={16} /></button>

        <div className="w-8 h-[1px] bg-slate-700/50 my-1" />

        <button onClick={() => setZoom((z) => Math.min(4, z + 0.1))} title="Zoom In" className="w-9 h-9 flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-700 rounded-lg transition-all"><ZoomIn size={16} /></button>
        <span className="text-[9px] font-mono text-slate-300 font-bold">{Math.round(zoom * 100)}%</span>
        <button onClick={() => setZoom((z) => Math.max(0.1, z - 0.1))} title="Zoom Out" className="w-9 h-9 flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-700 rounded-lg transition-all"><ZoomOut size={16} /></button>
        ...
        <button
          onClick={() => {
            if (clearArmed) {
              ...
              setClearArmed(true); setTimeout(() => setClearArmed(false), 5000);
            }
          }}
          className={`w-9 h-9 flex items-center justify-center rounded-lg transition-all border ${clearArmed ? "bg-red-600 border-red-400 text-white animate-pulse" : "bg-slate-800 border-slate-700 text-slate-400 hover:text-white"}`}
      ...
      <div className="absolute inset-0 z-0">
        ...
        <div
          className="absolute top-4 left-1/2 -translate-x-1/2 bg-[#1e293b]/90 backdrop-blur-xl border border-slate-700/50 shadow-2xl flex items-center z-40 transition-all duration-500 overflow-hidden h-14 w-[95%] md:w-[600px] rounded-2xl px-4 md:px-6 justify-between gap-2 md:gap-8"
        >
          <div className="flex items-center justify-between w-full h-full">
            <div className="flex items-center gap-1 md:gap-2 shrink-0">
              ...
              <h1 className="text-[10px] md:text-[15px] font-black text-white uppercase tracking-[0.1em] md:tracking-[0.25em] leading-none drop-shadow-sm whitespace-nowrap">TUDI WRAP CUSTOM DESIGN</h1>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <button onClick={exportCanvasToImage} className="bg-slate-800 hover:bg-slate-700 text-slate-300 px-2 py-1.5 md:px-3 md:py-1.5 rounded-xl text-xs font-bold flex items-center gap-1 md:gap-2 transition-all"><Download size={14} /> <span className="hidden sm:block">Export</span></button>
              <button onClick={handleAddToCart} disabled={submitStatus === 'submitting'} className={`${submitStatus === 'success' ? 'bg-green-600 hover:bg-green-500' : submitStatus === 'error' ? 'bg-red-600 hover:bg-red-500' : 'bg-indigo-600 hover:bg-indigo-500'} text-white px-2 py-1.5 md:px-3 md:py-1.5 rounded-xl text-xs font-bold flex items-center gap-1 md:gap-2 transition-all shadow-xl shadow-indigo-600/10 disabled:opacity-75`}><ShoppingCart size={14} className={submitStatus === 'submitting' ? "animate-pulse opacity-50" : ""} /> <span>{submitStatus === 'submitting' ? '...' : submitStatus === 'success' ? 'Added!' : submitStatus === 'error' ? 'Failed' : 'Add to Cart'}</span></button>
            </div>
          </div>
        </div>
        ...
        >
          <div id="main-canvas" ref={canvasRef} onPointerDown={(e) => { if (e.target.id === "main-canvas" && !spaceDown.current) { setSelectedIds([]); setIsDragging(true); dragInfo.current = { ids: [], type: "marquee", startX: e.clientX, startY: e.clientY, initialVals: {} }; setShowShapeMenu(false); setActiveColorEditId(null); setIsBottomPanelOpen(false); } }}
      ...
      {/* Main Tools Group (Select, Shapes, Text, Image) */}
      <div
        className={`fixed left-1/2 -translate-x-1/2 bg-[#1e293b]/90 backdrop-blur-xl border border-slate-700/50 shadow-2xl flex items-center justify-center z-[60] transition-all duration-500 h-14 rounded-2xl px-2 gap-2
          ${(isBottomPanelOpen || selectedIds.length > 0) ? 'bottom-[62vh] md:bottom-[360px]' : 'bottom-6 md:bottom-8'}`}
      >
        <button
          onClick={() => { setActiveTool("select"); setShowShapeMenu(false); }}
          title="Selection Tool (V)"
          className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 ${activeTool === "select" ? "bg-indigo-600 text-white shadow-lg" : "text-slate-400 hover:text-white hover:bg-slate-800"}`}
        >
        ...
        <button
          ...
          className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 ${activeTool === "direct-select" ? "bg-indigo-600 text-white shadow-lg" : "text-slate-400 hover:text-white hover:bg-slate-800"}`}
        >
          <MousePointerClick size={18} />
        </button>

        <div className="w-[1.5px] h-8 bg-slate-700/50 my-1 mx-1 shrink-0" />

        <div className="relative">
          <button
            onClick={() => setShowShapeMenu(!showShapeMenu)}
            title="Shapes"
            className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 ${showShapeMenu ? "bg-indigo-600 text-white shadow-lg" : "text-slate-400 hover:text-white hover:bg-slate-800"}`}
          >
            <Plus size={18} />
          </button>
          {showShapeMenu && (
            <div className="absolute bottom-[110%] left-1/2 -translate-x-1/2 bg-[#1e293b]/95 backdrop-blur-xl border border-slate-700/50 rounded-xl p-2 shadow-2xl z-[100] flex gap-2 animate-in slide-in-from-bottom-2">
              <button onClick={() => { setCurrentShape('rect'); addElement('rect'); setShowShapeMenu(false); }} className={`p-2 rounded-lg ${currentShape === 'rect' ? 'bg-indigo-500 text-white' : 'hover:bg-slate-700 text-slate-400'}`}><Square size={20} /></button>
              <button onClick={() => { setCurrentShape('circle'); addElement('circle'); setShowShapeMenu(false); }} className={`p-2 rounded-lg ${currentShape === 'circle' ? 'bg-indigo-500 text-white' : 'hover:bg-slate-700 text-slate-400'}`}><Circle size={20} /></button>
              <button onClick={() => { setCurrentShape('star'); addElement('star'); setShowShapeMenu(false); }} className={`p-2 rounded-lg ${currentShape === 'star' ? 'bg-indigo-500 text-white' : 'hover:bg-slate-700 text-slate-400'}`}><Star size={20} /></button>
              <button onClick={() => { setCurrentShape('triangle'); addElement('triangle'); setShowShapeMenu(false); }} className={`p-2 rounded-lg ${currentShape === 'triangle' ? 'bg-indigo-500 text-white' : 'hover:bg-slate-700 text-slate-400'}`}><TriangleIcon size={20} /></button>
            </div>
          )}
        </div>
        <button onClick={() => { addElement("text"); setShowShapeMenu(false); }} title="Text (T)" className="w-10 h-10 rounded-xl flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 transition-all duration-300"><Type size={18} /></button>
        <button onClick={activateEyedropper} title="Eyedropper (I)" className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 ${activeTool === 'eyedropper' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}><Pipette size={18} /></button>
        <button onClick={() => fileInputRef.current?.click()} title="Image" className="w-10 h-10 rounded-xl flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 transition-all duration-300"><ImageIcon size={18} /></button>
      </div>
        ...
        <div className={`bg-[#1e293b]/95 backdrop-blur-3xl border border-slate-700/50 shadow-2xl flex flex-col w-full h-full transition-all duration-500
          ...
          {!(isBottomPanelOpen || selectedIds.length > 0) ? (
            ...
            <div className="w-full h-full flex flex-col animate-in fade-in zoom-in duration-300">
              ...
              <div ref={propsPanelRef} className="flex-1 overflow-x-auto overflow-y-hidden p-4 md:px-6 custom-scrollbar scrollbar-hide snap-x snap-mandatory">
                {activeTab === "props" ? (
                  <div className="flex flex-row gap-4 md:gap-6 h-full items-start w-max pb-4 md:pb-2">
                    {selectedIdsCount > 1 ? (
                      <div className="bg-indigo-500/10 border border-indigo-500/30 rounded-2xl p-6 text-center shrink-0 w-[80vw] md:w-auto snap-center">
                      ...
                      <>
                        {selectedElement.type === "text" && (
                          <>
                            <section className="p-5 bg-slate-900/40 rounded-3xl border border-slate-800/60 shadow-lg shrink-0 w-[80vw] md:w-auto md:min-w-[240px] snap-center transition-all hover:border-indigo-500/30">
                            ...
                            <section className="p-5 bg-slate-900/40 rounded-3xl border border-slate-800/60 shadow-lg shrink-0 w-[80vw] md:w-auto md:min-w-[300px] snap-center transition-all hover:border-indigo-500/30">
                        ...
                        <section className="bg-slate-900/40 p-5 rounded-3xl border border-slate-800/60 shadow-lg shrink-0 w-[80vw] md:w-auto md:min-w-[300px] md:max-w-[340px] md:max-h-full md:overflow-y-auto custom-scrollbar snap-center transition-all hover:border-indigo-500/30">
                        ...
                        <section className="bg-slate-900/30 p-4 rounded-2xl border border-slate-800 shrink-0 w-[80vw] md:w-auto md:min-w-[280px] md:max-w-[320px] md:max-h-full md:overflow-y-auto custom-scrollbar snap-center">
                        ...
                        <section className="bg-slate-900/20 p-4 rounded-2xl border border-slate-800 shrink-0 w-[80vw] md:w-auto md:min-w-[220px] snap-center">
                      ...
                      <div className="flex flex-col md:flex-row gap-6 h-max md:h-full items-stretch md:items-start w-full md:w-max pb-4 md:pb-2">
                        <section className="bg-slate-900/40 p-5 rounded-3xl border border-slate-800/60 shadow-lg shrink-0 w-[80vw] md:w-auto md:min-w-[300px] md:max-w-[340px] snap-center transition-all hover:border-indigo-500/30">
